import { createPortal } from "react-dom"

const sprite = (
  <svg xmlns="http://www.w3.org/2000/svg">
    <symbol id="facebook" viewBox="0 0 24 24">
      <path
        fill="#2D9CDB"
        d="M17 2h-3a5 5 0 0 0-5 5v3H6v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3V2z"
      />
    </symbol>
    <symbol id="google" viewBox="0 0 24 24">
      <path
        fill="#4285F4"
        d="M21 12.207c0-.606-.054-1.18-.146-1.74h-8.659v3.456h4.958c-.222 1.134-.873 2.092-1.839 2.744v2.298h2.958C20.004 17.364 21 15.004 21 12.207z"
      />
      <path
        fill="#34A853"
        d="M12.195 21.195c2.483 0 4.56-.827 6.077-2.23l-2.958-2.298c-.827.551-1.877.889-3.119.889-2.398 0-4.429-1.617-5.157-3.801h-3.05v2.368c1.51 3.003 4.614 5.072 8.207 5.072z"
      />
      <path
        fill="#FBBC05"
        d="M7.038 13.755A5.336 5.336 0 0 1 6.748 12c0-.613.106-1.203.29-1.755V7.877h-3.05a9.089 9.089 0 0 0 0 8.246l3.05-2.368z"
      />
      <path
        fill="#EA4335"
        d="M12.195 6.444c1.357 0 2.567.468 3.525 1.38l2.621-2.62c-1.586-1.488-3.663-2.4-6.146-2.4-3.593 0-6.697 2.07-8.206 5.073l3.05 2.368c.727-2.184 2.758-3.8 5.156-3.8z"
      />
    </symbol>
    <symbol id="kakao" viewBox="0 0 24 24">
      <path
        fill="#F2C94C"
        d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"
      />
    </symbol>
  </svg>
)

export const GlobalSVGProvider = () => {
  return createPortal(sprite, document.body)
}
